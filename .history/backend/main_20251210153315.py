...
async def is_ca_related_question(question: str) -> bool:
    """
    Use a tiny LLM call to check if question is CA-related.
    Returns True if yes, False if no.
    """
    system = (
        "You are a classifier. Decide if the user question is about CA exams, "
        "CA syllabus, CA subjects (like taxation, accounts, law), CA practice, or audit. "
    ...
    result = await call_llm(messages)  # reuse your existing LLM wrapper
...
async def chat(req: ChatRequest, user=Depends(get_current_user)):
    ...
    # In future you can decide namespace based on user course.
    namespace = "CA_FINAL"
    ...
    if not matches:
        system_prompt = (
            ...
            "for this question. You must say you don't know."
        )
        messages = [
    ...
    for m in matches:
        ...
        block_header_parts = []
        if doc_title:
            block_header_parts.append(f"Document: {doc_title}")
        if chapter:
            block_header_parts.append(f"Chapter: {chapter}")
        if topic:
            block_header_parts.append(f"Topic: {topic}")
        if page_start:
            if page_end and page_end != page_start:
                block_header_parts.append(f"Pages: {page_start}-{page_end}")
            else:
                block_header_parts.append(f"Page: {page_start}")

        header = " | ".join(block_header_parts) if block_header_parts else "Context chunk"
        text = meta.get("text", "")
    ...
    system_prompt = (
        ...
        "If the exact answer is not present or is ambiguous, say you don't know and suggest consulting the official material.\n\n"
        f"Context:\n{context_str}"
    )
