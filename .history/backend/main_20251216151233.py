...
async def chat(req: ChatRequest, user=Depends(get_current_user)):
    async def safe_call(messages: List[dict], default: str) -> str:
        try:
    ...
    if is_basic_ca_question(req.message):
        ...
        answer = await safe_call(messages, "I couldn’t reach the model. Please try again in a moment.")
        # No external sources for basic conceptual answers (LLM-only)
        return ChatResponse(
            answer=answer,
            sources=[{"doc_title": "LLM (no source)", "note": "Conceptual answer generated by the assistant."}],
        )
    ...
    try:
        ...
        system_prompt = (
            "You are a helpful Indian CA tutor. Answer concisely using your CA knowledge. "
            "Mention that knowledge base retrieval is temporarily unavailable."
        )
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": req.message},
        ]
        answer = await safe_call(messages, "I couldn’t process this question right now. Please try again shortly.")
        return ChatResponse(answer=answer, sources=[{"doc_title": "LLM (fallback)", "note": "Answer without retrieval due to embedding error."}])

    # Namespace per course – for now, fixed to CA_FINAL
    namespace = "CA_FINAL"

    # 2) pinecone query (hybrid by content type)
    focus = detect_query_focus(req.message)
    ...
    try:
        ...
        res_text = {"matches": []}
    matches_map = {}
    for m in res_text.get("matches") or []:
        matches_map[m.get("id")] = m

    # targeted table/figure queries when relevant
    boost_table = 0.12 if focus in ("table", "mixed") else 0.0
    boost_figure = 0.12 if focus in ("figure", "mixed") else 0.0

    if boost_table > 0:
        res_table = index.query(
            vector=query_embedding,
            top_k=6,
            include_metadata=True,
            namespace=namespace,
        ...
        for m in res_table.get("matches") or []:
            m["score"] = (m.get("score") or 0) + boost_table
            matches_map[m.get("id")] = m
    ...
    if boost_figure > 0:
        ...
        for m in res_fig.get("matches") or []:
            m["score"] = (m.get("score") or 0) + boost_figure
            matches_map[m.get("id")] = m
        ...
        sources.append(
            {
                ...
                "chapter": chapter,
                "topic": topic,
                # optional: include table/figure urls if present in metadata
                "table_csv_url": meta.get("table_csv_url"),
    ...
    if req.mode == "discussion":
        ...
        system_prompt = (
            ...
            "When context includes tables or figures, explicitly reference them by 'Table' or 'Figure' and include page numbers.\n\n"
        ...
        system_prompt = (
            ...
            "If context contains TABLE or FIGURE references, mention them explicitly and quote any exact numeric values or captions from them.\n\n"
            f"Context:\n{context_str}"
        )
