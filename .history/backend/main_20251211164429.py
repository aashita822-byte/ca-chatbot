...
async def chat(req: ChatRequest, user=Depends(get_current_user)):
    ...
    if req.mode == "discussion":
        # Discussion mode: produce a dialogue between two personas: User A and User B
        style_instruction = (
            "Produce the answer as a short, focused dialogue between two personas: "
            "'User A' and 'User B'. Alternate turns (User A then User B, etc.). "
            "Keep the dialogue concise, exam-oriented, and accurate for CA Final students in India. "
            "Each turn should be 1-2 short sentences (not paragraphs). Aim for 4 to 8 exchanges total. "
            "Avoid hallucination — if the information is not present in the context, say so in one turn. "
        )

        # Additional instruction that enforces dialogue format + sources section
        system_prompt = (
            "You are an expert Indian CA tutor for CA Final students familiar with the ICAI syllabus. "
            "Use ONLY the context from the uploaded CA materials below.\n\n"
            f"{style_instruction}\n\n"
            "Strict output format requirements (must follow exactly):\n"
            "1) Provide the answer as a short dialogue between 'User A' and 'User B', alternating lines.\n"
            "   Example:\n"
            "     User A: First short sentence.\n"
            "     User B: Reply with another short sentence.\n"
            "     User A: Follow-up sentence.\n"
            "     User B: Final short point.\n\n"
            "2) Use 4–8 lines total (2–4 turns each). Do not include any extra headings or commentary outside the dialogue.\n\n"
            "3) After the dialogue, append a 'Sources Used:' section (exact heading) and list the document title, "
            "chapter/topic (if available), and page numbers for each source you relied on from the Context. "
            "If nothing matches clearly, write: 'No direct source found'.\n\n"
            f"Context:\n{context_str}"
        )
    else:
        # default: simple Q&A / exam-oriented
        style_instruction = (
        ...
        system_prompt = (
            "You are an expert Indian CA tutor for CA Final students, deeply familiar "
            "with the ICAI syllabus, past papers, and Indian accounting & tax practice. "
            "Use ONLY the context from the uploaded CA materials below. "
            "If the exact answer is not present or is ambiguous, say you don't know and "
            "recommend referring to the official ICAI material or the cited sources.\n\n"
            f"{style_instruction}\n\n"
            f"Context:\n{context_str}"
        )

    # Ensure final instruction to append Sources Used (applies to both modes)
    if "Sources Used" not in system_prompt:
        system_prompt = (
            system_prompt
            + "\n\nAfter giving your answer, ALWAYS add a section titled:\n'Sources Used'\nList the exact document titles, chapters, topics and page numbers from the context that were relevant to the answer. If nothing matches clearly, write: 'No direct source found'.\n"
        )

    messages = [{"role": "system", "content": system_prompt}]
    if req.history:
        # backend expects simple dicts for history
