...
// Quick suggested prompts tailored for CA students (you can expand)
const SUGGESTED_PROMPTS = [
  "Explain treatment of depreciation under Ind AS (brief).",
  "Conditions for claiming input tax credit under GST.",
  "Summarise section 80C deductions with limits.",
  "Explain framework for financial reporting under Ind AS (short).",
];
...
const Chat: React.FC = () => {
  ...
  // control which message's sources are open
  ...
  const copyToClipboard = async (text: string) => {
    try {
      ...
      // small visual feedback - you can replace with toast
  ...
  return (
    ...
    >
      <div className="chat-card-header">
        <div className="header-left">
          ...
          <p className="chat-subtitle">
            Grounded answers from uploaded CA Final study PDFs — concise, exam-focused.
          </p>
        </div>

        <div className="header-right">
          <div className="exam-badge" aria-hidden>
            CA Final
          </div>
          ...
          <div className="chat-header-actions">
            <div className="chat-mode-toggle" role="tablist" aria-label="Answer mode">
              <button
                ...
                className={mode === "qa" ? "chat-mode-btn chat-mode-btn-active" : "chat-mode-btn"}
                onClick={() => setMode("qa")}
              >
                Simple Q&amp;A
              </button>
              <button
                ...
                className={mode === "discussion" ? "chat-mode-btn chat-mode-btn-active" : "chat-mode-btn"}
      ...
      <div className="suggested-chips" aria-hidden>
        {SUGGESTED_PROMPTS.map((p, idx) => (
          <button
            key={idx}
            className="chip"
            onClick={() => useSuggested(p)}
      ...
      <div className="chat-messages" aria-live="polite">
        {messages.length === 0 && (
          <div className="chat-empty">
            ...
            <p className="empty-sub">Try one of the suggested prompts above or type your own.</p>
          </div>
        )}
        ...
        {messages.map((m, i) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className="chat-bubble-role">{m.role === "user" ? "You" : "Tutor"}</div>
                ...
                {dialogue ? (
                  <div className="dialogue-block">
                    {dialogue.map((ln, idx) => (
                      ...
                      >
                        <span className="dialogue-speaker">{ln.speaker ? `User ${ln.speaker}: ` : ""}</span>
                ...
                <div className="chat-bubble-footer">
                  {m.ts && <div className="chat-bubble-time">{new Date(m.ts).toLocaleTimeString()}</div>}
                        ...
                        return (
                          <li key={idx} className="chat-source-item">
                            ...
                            <div className="chat-source-meta">
                              {chapterTopic && <span className="chat-source-meta-item">{chapterTopic}</span>}
